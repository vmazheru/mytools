...
/**
 * A {@code Decorator} is an object which changes the behavior of a given
 * function in a certain way.
 *
 * <p>
 * For example, adding a retry logic to your method can be done by creating a
 * decorator. Such a decorator can later be used with any method. The other
 * examples include aspects like exception handling, batching, throttling,
 * transaction handling, etc.
 *
 * <p>
 * {@code Decorator} interface defines a number of overloaded {@code decorate()}
 * methods each of which takes an object of a specific function type and returns
 * a function of the same type. The returned function can be called normally as
 * the original function, but will expose some additional behavior.
 *
 * <p>
 * To implement a decorator, one has to implement only one {@code decorate()}
 * method, which takes a parameter of type {@code BiFunction}. The other
 * {@code decorate()} methods will delegate to it by converting their input
 * functions into a {@code BiFunction} and back.
 *
 * <p>
 * The following snippet shows how to create a decorator which executes its
 * wrapped method after a delay of 1 second.
 *
 * <pre>
 * {
 *     &#64;code
 ...
 *             // the parameter here is Runnable not a BiFunction,
 *             // but still works
